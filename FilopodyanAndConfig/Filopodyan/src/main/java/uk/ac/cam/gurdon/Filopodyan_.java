...
import uk.ac.cam.gurdon.ext.CommandParser;
...
public class Filopodyan_ implements Command {
            ...
            for (int t = t0; t <= t1; t++) {
                ...
                for (int i = 0; i < split.length; i++) {
                    ...
                    if (bounds1.x == 0 && bounds1.y == 0) { //reset ROI location if it was lost - workaround for bug in ShapeRoi.getRois()
                        ...
                        if (bgui.verbose)
                            bgui.log.print(title, "Resetting ROI location in frame " + t + " : " + bounds0.x + "," + bounds0.y);
                    }
    ...
    public void filopodia(boolean prev, FilopodyanProcessor filoProcessor) {
        try {
            ...
            if (bgui.verbose) {
                bgui.log.print(title, "Mapping processes in " + title);
            }
            if (!prev) {
            ...
            if (!batch) {
                ...
                userRoi = imp.getRoi();
                if (userRoi != null && userRoi.getType() > 3) {
                    userRoi = null;
                }    //ignore if not an area
            ...
            if (bgui.adaptive) {
                ...
                if (bgui.verbose) {
                    bgui.log.print(title, "Running adaptive thresholding");
                }
                ...
                if (bgui.verbose) {
                    bgui.log.print(title, "Running Laplacian of Gaussian");
                }
                map = filoProcessor.process(imp, bgui.mapC, tStart, tEnd, bgui.sigma, bgui.threshold, bgui.verbose);
            }
            ...
            if (userRoi != null) {
                if (bgui.verbose) {
                    IJ.log("Analysing user-defined ROI");
                }
                map.setRoi(userRoi);
            ...
            if (bgui.verbose) {
                bgui.log.print(title, "Running binary segmentation");
            }
            ...
            for (int t = tStart; t <= tEnd; t++) {
                ...
                if (bgui.verbose) {
                    bgui.log.print(title, "Analysing objects at T" + t);
                }
                ...
                if (bgui.verbose) {
                    bgui.log.print(title, "Got projections at T" + t + " for " + title);
                }
                ...
                if (bgui.verbose) {
                    bgui.log.print(title, "Got growth cone body");
                }
                ...
                imp.setPosition(bgui.measureC, 1, t);
                if (bgui.verbose) {
                    bgui.log.print(title, "Processing " + split.length + " objects...");
                }
                Tipper tipper = new Tipper();
                for (int f = 0; f < split.length; f++) {
                    if (bgui.verbose) {
                        bgui.log.print(title, "Object " + f);
                    }
                    if (onEdge(split[f])) {
                        continue;
                    }
                    ...
                    if (bgui.verbose) {
                        bgui.log.print(title, "Finding process tip");
                    }
                    ...
                    if (processRoi.getType() == Roi.COMPOSITE) {
                        ...
                        while (processRoi.getType() == Roi.COMPOSITE && n < 15) {
                            if (bgui.verbose) {
                                bgui.log.print(title, "composite join " + n + " Roi length = " + processRoi.getLength());
                            }
                            if (processRoi.getLength() < 1) {
                                processRoi = clone;
                                break;
                            }
    ...
    public void doOverlay() {
        try {
            ...
            if (bgui.verbose) {
                bgui.log.print(title, "Making Overlay for " + title);
            }
            firstTrackIndex = Integer.MAX_VALUE;
            for (int t = 0; t < filo.size(); t++) {    //make overlay
                if (bodyRoiArr[t] == null || filo == null || filo.size() == 0) {
                    continue;
                }
    ...
    public void output(final ArrayList<ArrayList<FiloPod>> backPass) {
        try {
            if (!batch) {
                bgui.workFrame.setVisible(true);
            }
            setImageVisible(false);
            bgui.setLabel("analysing objects<br>" + title);
            if (bgui.verbose) {
                bgui.log.print(title, "Analysing objects...");
            }
            filo = backPass;
            ...
            for (int i : trackIndices) {
                ...
                if (bgui.verbose) {
                    bgui.log.print(title, "Measuring track " + i);
                }
                    ...
                    try {
                        ...
                        for (int f = 0; f < filo.get(t).size(); f++) {
                            ...
                            if (part.getIndex() == i) {
                                ...
                                }    //ignore exception caused by zero area ShapeRoi, baseBackgroundRoi remains null
                                ...
                                if (t > 0) {
                                    dlArr[t] = lengthArr[t] - lengthArr[t - 1];
                                }
                ...
                if (!bgui.filoTable && !bgui.coordTable) {
                    continue;
                }
                try {
                    int rtrow = -1;
                    ...
                    for (int ti = tStart; ti <= tEnd; ti++) {
                        ...
                        if (tipCoordArr[t] != null) {    //if it has a tip at this t
                            ...
                            if (t >= tStart && ctrow > 0) {
                                ...
                                if (tipCoordArr[t] == null) {
                                    continue;
                                }    //index wasn't assigned for this frame
            ...
            if (bgui.filoTable) {
                ...
                for (int c = 0; c < head.length; c++) {
                    for (int r = 0; r < filoRT.getCounter(); r++) {
                        ...
                        if (value == 0) {
                            if (filoRT.getValue("dT", r) == 0 && (head[c].startsWith("dT") || head[c].startsWith("T ("))) {
                                continue;
                            }
...
//                    filoRT.show(sanTitle + " Filopodia");
            ...
            if (batch) {
                try {
                    ...
                    if (bgui.filoTable) {
                        filoRT.saveAs(tablePath + sanTitle + "_Filopodia.csv");
                    }
                    if (bgui.bodyTable) {
                        bodyRT.saveAs(tablePath + sanTitle + "_Bodies.csv");
                    }
                    if (bgui.coordTable) {
                        coordRT.saveAs(tablePath + sanTitle + "_Coordinates.csv");
                    }
                    if (bgui.filoTable || bgui.bodyTable || bgui.coordTable) {
                        bgui.log.print(title, title + " tables saved in " + tablePath);
                    }
            ...
            if (bgui.log != null) {
                ...
                bgui.log.setVisible(CommandParser.SHOW);
            }
        } catch (Exception e) {
            IJ.log(e.toString() + "\n~~~~~\n" + Arrays.toString(e.getStackTrace()).replace(",", "\n"));
        }
    ...
    public String toString() {
        return "Filopodyan_{" +
                ...
                ", backRoi=" + backRoi +
                ", ol=" + ol +
                ", bodyRoiArr=" + Arrays.toString(bodyRoiArr) +
                ", frameBackgroundRoiArray=" + Arrays.toString(frameBackgroundRoiArray) +
                ", boundaryBackgroundRoiArray=" + Arrays.toString(boundaryBackgroundRoiArray) +
                ", localBackgroundRois=" + localBackgroundRois +
                ", bodyMean=" + Arrays.toString(bodyMean) +
                ", batch=" + batch +
                '}';
    }
}
