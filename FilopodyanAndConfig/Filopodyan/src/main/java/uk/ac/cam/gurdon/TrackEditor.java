        ...
        try {
            if (from != to) {
                ...
                if (parent.bgui.verbose) {
                    FilopodyanLog.get().print(parent.imp.getTitle(), "Index " + from + " changed to " + to);
                }
//			if(tracklog){ edits.add(TrackEdits.Op.CHANGE, from, to, min); }
    ...
    public void update() {
        ...
        parent.update(filo);
        if (tracklog) {
            edits.add(TrackEdits.Op.UPDATE);
        }
    }
    ...
    public void delete(int trackI, int t1, int t2) {
        try {
            ...
            if (parent.bgui.verbose) {
                FilopodyanLog.get().print(parent.imp.getTitle(), "Deleted " + t1 + " to " + t2 + " from track " + trackI);
            }
            if (tracklog) {
                edits.add(TrackEdits.Op.DELETE, trackI, t1, t2);
            }
        } catch (Exception e) {
            IJ.log(e.toString() + "\n~~~~~\n" + Arrays.toString(e.getStackTrace()).replace(",", "\n"));
        }
    ...
    public void delete(PartNode del, boolean wholeTrack, boolean confirm) {
        try {
            if (!wholeTrack) {
                ...
                for (int t = 0; t < filo.size(); t++) {
                    for (int p = 0; p < filo.get(t).size(); p++) {
                        if (filo.get(t).get(p).getIndex() == del.index && t == del.t) {
                            filo.get(t).remove(p);
                            if (tracklog) {
                                edits.add(TrackEdits.Op.DELETE, del.index, t + 1, t + 1);
                            }
                            if (parent.bgui.verbose) {
                                FilopodyanLog.get().print(parent.imp.getTitle(), "Deleted object from track " + del.index + " at T" + (t + 1));
                            }
                            break outer;
                        }
                    }
                ...
                if (parent.bgui.verbose) {
                    FilopodyanLog.get().print(parent.imp.getTitle(), "Deleted track " + del.index);
                }
            }
        } catch (Exception e) {
            IJ.log(e.toString() + "\n~~~~~\n" + Arrays.toString(e.getStackTrace()).replace(",", "\n"));
    ...
    public void sequentialise() {
        try {
            if (parent.bgui.verbose) {
                FilopodyanLog.get().print(parent.imp.getTitle(), "Sequentialising track indices");
            }
    ...
    public void restore() {
        try {
            ...
            scroll.repaint();
            if (tracklog) {
                edits.add(TrackEdits.Op.RESTORE);
            }
            if (parent.bgui.verbose) {
                FilopodyanLog.get().print(parent.imp.getTitle(), "Restored original tracks");
            }
        } catch (Exception e) {
            IJ.log(e.toString() + "\n~~~~~\n" + Arrays.toString(e.getStackTrace()).replace(",", "\n"));
        }
    ...
    private void setIndex(PartNode part) {
        try {
            ...
            try {
                ...
                IJ.error(toStr + " is not an integer");
                return;
            }
    ...
    public void run() {
        try {
                            ...
                            button.addActionListener(new ActionListener() {
                                public void actionPerformed(ActionEvent ae) {
                                    ...
                                    try {
                                        ...
                                        IJ.error("Please enter integers for timepoint range");
                                    }
            ...
            qdOffTick = new JRadioButton("off", false);
            qdOffTick.setBackground(Color.WHITE);
            deleteGroup.add(qdOffTick);
            buttonPan.add(qdOffTick);
            qdOffTick.addActionListener(qdListen);
            qdObjectTick = new JRadioButton("objects", false);
            qdObjectTick.setBackground(Color.WHITE);
            deleteGroup.add(qdObjectTick);
            buttonPan.add(qdObjectTick);
            qdObjectTick.addActionListener(qdListen);
            qdTrackTick = new JRadioButton("tracks", false);
            qdTrackTick.setBackground(Color.WHITE);
            deleteGroup.add(qdTrackTick);
            buttonPan.add(qdTrackTick);
            qdTrackTick.addActionListener(qdListen);
    ...
    private JScrollPane makePanel() {
        ...
        try {
            ...
            if (parent.bgui.verbose) {
                FilopodyanLog.get().print(parent.imp.getTitle(), "Constructing Track Editor for " + filo.size() + " frames");
            }
            for (int t = 0; t < filo.size(); t++) {
                for (int p = 0; p < filo.get(t).size(); p++) {
            ...
            MouseAdapter ma = new MouseAdapter() {
                ...
                public void mouseExited(MouseEvent me) {
                    try {
                        if (rcMenu != null && rcMenu.isVisible()) {
                            return;
                        }
                        if (startNode != null) {
                            startNode.setColour(Color.GRAY);
